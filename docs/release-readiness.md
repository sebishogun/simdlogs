...
| `LOGS-V1-02` | in-progress | Push and observed CI: the workflows have never been observed running; separate user push permission is required at operation time, so the task stops at the evidence checkpoint until that permission is given | observed CI result | CI observed |
