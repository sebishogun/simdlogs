...
| `/select/logsql/hits` | `federatedHits` decodes `{"hits":[{"_time":..,"hits":..}]}` — the OLD object shape — but the backend `selectHits` now answers the dense series shape `{"hits":[{"fields":..,"timestamps":[..],"values":[..],"total":N}]}`. No item has `_time`/`hits`, so the merged map stays empty: **the router answers `{"hits":[]}` for any non-empty store**. | **broken** (empty answer) |
...
| `/select/logsql/field_values`, `stream_field_values`, `field_names`, `stream_field_names` | `federatedValueCounts`/`federatedStrings`: backends answer `{"values":[...]}`/string lists; hits summed per value (or union for names), sorted. | works |
